...
public:
    ...
    int         nTestResult;    // 测试案例执行结果
    double      nFailed;        // 测试失败案例数
    double      nPassed;        // 测试成功案例数
};

// 单元测试, 所有测试案例加入vector中 (观察者模式), TestCase类为观察者
class UnitTest {
public:
    // 获取一个案例
    ...
    // 案例加入vector
    TestCase* RegisterTestCase(TestCase* testcase) {  // 设计模式：
    ...
    void Run() {
        ...
        for (auto it : testcases_) {
            ...
            std::cout << " " << testcase->nPassed << " / "
                      ...
                      << testcase->nPassed / (testcase->nFailed + testcase->nPassed) * 100 << "%  )\n";

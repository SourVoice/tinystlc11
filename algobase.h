...
OutputIter unchecked_move(InputIter first, InputIter last,
    ...
    // TODO: iterator_category()和iterator_traits区别
}
...
RandIter2 unchecked_move_backward_cat(RandIter1 first, RandIter2 last,
    ...
    for (auto n = last - first; n > 0; n--, --last, --first)
        *result = MySTL::move(*last) return result;
}

template <class InputIter, class OutputIter>
OutputIter unchecked_move_baackward(InputIter first, InputIter last,
                                    OutputIter result) {
...
typename std::enable_if<
    ...
    std::is_trivially_move_assignable<Up>::value,
    Up*>::type
unchecked_move_baackward(Tp* first, Tp* last, Up* result) {
    const size_t n = static_cast<size_t>(last - first);
    if (n != 0) {
        result -= n;
        std::memmove(result, first, n * sizeof(Up));
...
void fill_cat(RandIter first, RandIter last, const T& value, MySTL::random_access_iterator_tag){
    fill_n(first, last, valu, iterator_category(first));
}
